...
DIR_LOG_HADOOP=/home/hadoop/bigdata/logs/sistema_a
DIR_INPUT_HDFS=/user/hadoop/input/sistema_a


# Declarando variáveis para os diretórios do HDFS e para o Job do Sqoop
DIR_INPUT=/user/hadoop/input/sistema_a; DIR_OUTPUT=/user/hadoop/output
BD_CONEXAO=jdbc:mysql://192.168.1.109:3306/tcc_hadoop; BD_USUARIO=root; BD_TABELA=sqoopinseredados
#BD_CONEXAO=jdbc:mysql://192.168.56.1:3306/tcc_hadoop; BD_USUARIO=root; BD_TABELA=sqoopinseredados

echo ''
echo '------------------------------------------------------------------------------------------------------------------------------------------------------'
echo '|                                               Iniciando Inclusão de Arquivos de Log do Sistema A no HDFS                                           |'
echo '------------------------------------------------------------------------------------------------------------------------------------------------------'
echo ''
...
echo "$(date +%y/%m/%d" "%H:%M:%S) INFO Executando Job Extrair Info Log Sistema A"
echo '------------------------------------------------------------------------------------------------------------------------------------------------------'
echo ''
