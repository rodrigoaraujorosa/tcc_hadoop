 ...
 */
public class JobExtracaoSysADriver {
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != 3) {
			System.err.println("Usar: JobExtracaoSysA <regex> <dir_entrada> <dir_saida>");
			System.exit(2);
		}
		conf.set("mapregex", otherArgs[0]); // Define a expresão regular informada por parâmetro
		
		Job job = new Job(conf, "Extrair Info Log Sistema A");       // Cria um novo job mapreduce
		job.setJarByClass(JobExtracaoSysADriver.class);              // Define a classe de execução
		job.setMapperClass(JobExtracaoSysAMapper.class);             // Define a classe Mapper
